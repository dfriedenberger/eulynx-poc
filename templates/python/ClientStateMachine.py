...
class ClientStateMachine:
	...
	def wait(self):
		raise NotImplementedError("wait not implemented yet")
	...
	def run(self):
		...
		sm = StateMachine(self.state_init, [
			...
			self.state_connected	: [
				...
				(self.is_send_data,self.send,self.state_connected),
			],
			self.state_idle	: [
				(self.is_true,self.connect,self.state_connected),
			],
			self.state_init	: [
				(self.is_true,self.initialize,self.state_idle),
			],
			self.state_handleRecvData	: [
				(self.is_timeout,self.timeout,self.state_connected),
				(self.is_recv_data,self.handle_callback,self.state_connected),
			],
			self.state_closed	: [
				(self.is_true,self.wait,self.state_init),
			],
		})
